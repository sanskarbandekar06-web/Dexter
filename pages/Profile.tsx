...
import {
  ...
  ShieldAlert, UserMinus, Calendar
} from 'lucide-react';
...
const ProfilePage: React.FC<ProfilePageProps> = ({ 
  ...
  // Streak gives stability, Daily Score pushes it higher temporarily or permanently if consistent.
  const streakPoints = profile.streak * 100;
  const currentScore = userData.score || 0;
  const totalExperience = streakPoints + currentScore;
  const levelThreshold = 250; 
  
  // Use persisted level if available, otherwise calculate on fly
